...
export function RecordSinglesPage() {
  ...
  const runCoaching = async (sessionId: string) => {
    const newSession = sessions.find((s) => s.id === sessionId)
      ?? { id: sessionId, type: 'singles' as const, date, sets, memo: memo.trim(), createdAt: new Date().toISOString() }

    // 直近5件を past context として渡す
  ...
  const handleSubmit = async () => {
    ...
    setErrorMsg('')
    try {
      const session = addSession({ type: 'singles', date, sets, memo: memo.trim(), opponentId })
  ...
  const handleSkip = () => {
    // エラー後にスキップ: すでに session は保存済みなので最新IDでリダイレクト
  ...
  return (
    <div className="mx-auto max-w-md px-5 pt-[48px]">
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-[28px] font-extrabold leading-[1.3] tracking-[-0.02em]">シングルスを記録</h1>
      </div>
      ...
      <div className="space-y-8">
        ...
        <PersonPicker value={opponentId} onChange={setOpponentId} label="相手" />
